...
// Renders the generated map at the same normalized position as the live lens and
// lets the user drag it, so moving the map moves the component and vice versa.
...
// ─── Slider configs ───────────────────────────────────────────────────────────
// Order and defaults match the Aave article exactly so values can be compared
// one-to-one. The 2-column layout pairs them left→right, row by row.
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-6xl gap-16 pb-16">
      ...
      <div className="flex flex-col items-center w-full max-w-4xl">
        <div className="flex flex-col md:flex-row w-full gap-8">
          ...
          <div className="flex-1">
            <div className="w-full h-[360px] rounded-[24px] overflow-hidden border border-gray-100 dark:border-gray-800 bg-[#f4f0ff] dark:bg-gray-950 relative">
              ...
              >
                <div className="absolute inset-0" style={{ backgroundImage: "linear-gradient(to right, rgba(100,100,255,0.18) 1px, transparent 1px), linear-gradient(to bottom, rgba(100,100,255,0.18) 1px, transparent 1px)", backgroundSize: "40px 40px" }} />
              </LiquidGlass>
            </div>
          </div>
        ...
        {/* Description below previews, above controls — matching the article */}
        <p className="mt-5 mb-1 text-sm text-gray-500 text-center max-w-lg">
          On the left is the refracted result, on the right the map that drives it.
        </p>

        {/* Unified 2-column control panel */}
        <DebugPanel
          ...
          columns={2}
        >
          <div className="flex justify-center pt-3 border-t border-white/[0.06] mt-1">
        ...
        <div className="flex flex-col items-center w-full max-w-sm">
          <div className="w-full h-[320px] rounded-xl overflow-hidden border border-gray-800 bg-gray-950">
            <LiquidGlass
              ...
              {...imageExp.props}
              onLensMove={(x, y) => {
                imageExp.patch("lensX", x);
                imageExp.patch("lensY", y);
              }}
            >
              <div
        ...
        <div className="flex flex-col items-center w-full max-w-sm">
          <div className="w-full h-[320px] rounded-xl overflow-hidden border border-gray-800 bg-gray-950 flex items-center justify-center relative">
            ...
            <LiquidGlass
              ...
              {...textExp.props}
              onLensMove={(x, y) => {
                textExp.patch("lensX", x);
                textExp.patch("lensY", y);
              }}
            >
        ...
        <div className="flex flex-col items-center w-full max-w-sm">
          <div className="w-full h-[320px] rounded-xl overflow-hidden border border-gray-800 bg-gray-950 flex items-center justify-center relative p-8">
            <LiquidGlass
              ...
              {...cardExp.props}
              onLensMove={(x, y) => {
                cardExp.patch("lensX", x);
                cardExp.patch("lensY", y);
              }}
            >
              <div className="absolute inset-8 rounded-2xl bg-gradient-to-br from-blue-500 via-purple-500 to-pink-500 p-6 flex flex-col justify-between shadow-2xl">
                <div className="flex justify-between items-start">
                  ...
                  <div className="px-3 py-1 rounded-full bg-white/20 backdrop-blur-md text-xs font-medium text-white">
                    Pro
                  </div>
                </div>
                <div>
